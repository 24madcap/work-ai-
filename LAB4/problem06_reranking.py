...
# Uses real data from com_q_a.txt (hardware category)
from data_loader import load_qa

GENERIC_TERMS = ["คอมพิวเตอร์", "ทำงาน"]
SPECIFIC_TERMS = ["เมนบอร์ด", "ซ็อกเก็ต"]
...
def run():
    ...
    print("Before Re-ranking (Top 6 from First-stage: generic terms 'คอมพิวเตอร์','ทำงาน'):")
    for d in first:
        print(f"  score={first_stage(d)} | {d['question']}")

    print("\nAfter Re-ranking (extra weight for specific terms, e.g. 'เมนบอร์ด'):")
    for d in second:
