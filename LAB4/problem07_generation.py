...
# Uses a real answer from com_q_a.txt (PEP, which has a medically critical time window)
from data_loader import load_qa


def find_entry(data):
    return next(d for d in data if d["lang"] == "ทางการ" and d["question"].startswith("คอมพิวเตอร์"))


def bad_generator(context):
    # Simulated failure: the generator unknowingly changes a critical number/condition
    return context.replace("อิเล็กทรอนิกส์", "สิ่งมีชีวิต").replace("ประมวลผล", "หายใจ")
